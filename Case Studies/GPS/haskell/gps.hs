...
makeInitialState userInput = do
    newIORef userInput

packUserInput userInput = map (DT.unpack) $ DT.splitOn (DT.pack ", ") (DT.pack userInput)
...
main = do
  (inputStates : inputGoals : _) <- getArgs
  if (inputStates == inputGoals)
    then do
      putStrLn "Already at goal state"
    else do
      states <- makeInitialState $ packUserInput inputStates
      let goals = packUserInput inputGoals
